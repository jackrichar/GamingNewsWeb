...
const CustomSelectBox = (
    ...
    return (
        <div className="Custom-Select-Box" onClick={()=> setIsShow(!isShow)} ref={ClickOut}>
            <div className={`Custom-Select-Box-Text`}>
                <span>{!selectedItem ? Placeholder : selectedItem}</span>
            </div>
            <div className="Custom-Select-Box-Arrow">
                <RightArrowIcon style={isShow ? {rotate: "90deg"} : {rotate: "0deg"}}/>
            </div>
            <div className={`Custom-Select-Box-Drop-Down ${isShow ? "Show-Up" : null}`}>
                <ul className="Item-List-Background" onClick={(e) => handleSelect(e)}>
                    {
                        Value.map((item, index) => (
                            <li className="Item-List" key={index}>
                                {
                                    !Multiple ?
                                        <button data-name={item} className={`Item-List-Button ${selectedItem === item ? "Item-List-Button-Active" : null}`}><span>{item}</span></button>
                                        :
...
CustomSelectBox.propTypes = {
    ...
    initialSelected: PropTypes.arrayOf(PropTypes.oneOfType([PropTypes.string, PropTypes.number])),
};
